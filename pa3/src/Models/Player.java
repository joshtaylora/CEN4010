...
public class Player {
	...
	public void purchaseDeed(Deed property) {
		...
		account -= cost;
		playerDeeds[property.getPropertySet()].addProperty(property); // we need some way of knowing which set is which
																		// i.e SOMEVALUE
	...
	/**
	 * this method is to initiate a trade to another player, trading properties,
	 * money or both. Starts off by prompting the player for the object of the
	 * player they would like to trade with. After that the current players
	 * properties and money will pull up and they can choose what they want to
	 * trade. After that the other players properties and money will pop up and the
	 * current player then chooses what they want to trade in return. Then all the
	 * trade information will be sent to the other player using a method through the
	 * Controller. They the other player either accepts or rejects. Upon accepting
	 * the method will transfer all properties and money. Upon rejection the method
	 * will just return.
	 * 
	 * @param tradePlayer
	 * @param cPlayerMoney
	 ...
	 * @param tPlayerDeeds
	 */
	public void initiateTrade(Player tradePlayer, int cPlayerMoney, int tPlayerMoney, Deed cPlayerDeeds[],
			Deed tPlayerDeeds[], boolean tradeResults) {
		int i;
	...
	/**
	 * TODO add a method in controller that splits the inputs from trade into
	 * playerA and playerB
	 */

	/**
	 * This method is to receive a trade from another player through th controller,
	 * be it properties, money or both. Takes in the result from the controller and
	 * returns the response back through.
	 * 
